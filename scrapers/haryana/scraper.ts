...
const BASE_URL = 'https://etenders.hry.nic.in'
...
export async function scrape(): Promise<ParsedTender[]> {
  ...
  try {
    ...
    // Wait for tender list to render (NICGEP loads via JS on some views)
    ...
    while (pageNum <= MAX_PAGES) {
      ...
      tenders.push(...parsed)
      console.log(`[Haryana] Page ${pageNum}: ${parsed.length} tenders`)

      // NICGEP pagination selectors
      const nextBtn = await page.$(
        'a[title="Next Page"], a:has-text("Next"), .nextPage, [aria-label="Next page"], input[value="Next"], .paginator-next-link'
      )
      if (!nextBtn) break
