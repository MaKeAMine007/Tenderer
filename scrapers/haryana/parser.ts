...
export function parseHtml(html: string, baseUrl: string): ParsedTender[] {
  ...
  // Haryana (NICGEP) uses standard tr rows; class-based regex as fallback for any portal variants
  const classRowRegex =
    /<tr[^>]*class="[^"]*(?:row|trow|data|odd|even|portlet-section-body|results)[^"]*"[^>]*>([\s\S]*?)<\/tr>/gi
  const allRowRegex = /<tr[^>]*>([\s\S]*?)<\/tr>/gi

  let usedRegex = classRowRegex
  let sampleMatch = classRowRegex.exec(html)
  classRowRegex.lastIndex = 0

  if (!sampleMatch) {
    usedRegex = allRowRegex
  }

  let match: RegExpExecArray | null
  while ((match = usedRegex.exec(html)) !== null) {
    ...
    // Skip header rows (NICGEP and Liferay both use td for headers)
    ...
    const tender: ParsedTender = {
      ...
      department: col(cells, 'department', 4) || 'Government of Haryana',
      state: 'Haryana',
