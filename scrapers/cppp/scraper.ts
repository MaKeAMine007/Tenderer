...
const BASE_URL = 'https://eprocure.gov.in'
const PORTAL_URL = `${BASE_URL}/eprocure/app?page=FrontEndLatestActiveTenders&service=page`
...
export async function scrape(): Promise<ParsedTender[]> {
  ...
  try {
    ...
    while (pageNum <= MAX_PAGES) {
      ...
      tenders.push(...parsed)
      console.log(`[CPPP] Page ${pageNum}: ${parsed.length} tenders`)

      const nextBtn = await page.$('a[title="Next Page"], a:has-text("Next"), .nextPage')
      if (!nextBtn) break
